...
Y_0 = '/Users/jamie/Downloads/numerai_live_data.csv'
X = pd.read_csv( X_0 )
Y = pd.read_csv( Y_0 )
...
preds = 'New_NNtest-' + str(cycles) + '-cycles-live_feature.csv'
df.to_csv(preds, columns = header, index=False)
test = pd.read_csv( Y_0 )
preds = pd.read_csv( preds )
test_ids = test[['t_id']]
#print preds.iloc[:10]
#print test_ids.iloc[0]
total = pd.concat(( test_ids, preds), axis = 1)
total.to_csv('New_NNout-' + str(cycles) + '-cycles-live_feature.csv', index=False)
